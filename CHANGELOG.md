...
### 新增
- **第三關：把專案跑起來** - 前後端架構與 Docker 入門
  - 前端、後端、資料庫三層架構互動教學
  - Docker 容器化概念互動簡報
  - 系統架構互動式視覺化展示
- **海克絲背景故事**：新增主角詢問海克絲工程師背景的對話劇情
- **教練新表情**：新增 `blush`（臉紅）、`proud`（驕傲）表情

### 變更
- **第二關場景更新**：場景從 `normal` 改為 `meeting`（會議室）
- **第三關場景更新**：場景從 `normal` 改為 `sport`（運動場景）
- **比喻調整**：前後端架構說明從餐廳比喻改為電商比喻，與互動簡報一致
- **Git 互動簡報深色主題**：將淺色主題改為深色主題，風格統一

### 修復
- 調整互動簡報中的中文標籤（網頁商城/Browser、訂單處理中心/Server、商品倉庫/Storage）
